...
{
	t_arenablock *first = malloc(sizeof(t_arenablock) + INITIAL_SIZE);
	if (!first)
		return (FAIL);
	first->next = NULL;
...
void	init_arena(t_minirt *minirt)
{
	minirt->arena = malloc(sizeof(t_arena));
	// if (!minirt->arena)
		// exit_cleanup_error(minirt, "malloc");
	init_arena_values(minirt->arena);
	arena_create_first_block(minirt->arena);
	// if (above_fails)
		// exit_cleanup_error(minirt, "malloc");
}
